...
mur_harmonise <- with(mur, 
                      harmonise(X = pixellate(gold, dimyx = 256) > 0,
                                D = distfun(faults),
                                G = greenstone))
```
...
suppressWarnings({
test_df$`Gold deposit` <- as.factor(test_df$`Gold deposit`)
ref_mat <- matrix(test_df$`Gold deposit`, nrow = 172, ncol=114)
ref_mat_im <- im(ref_mat)
plot(ref_mat_im, main = "Reference")

log_mat <- matrix(logit.results.smote, nrow=172, ncol=114)
log_mat_im <- im(log_mat)
plot(log_mat_im, main = "Logistic Regression")

svm_mat <- matrix(svm.results.smote, nrow=172, ncol=114)
svm_mat_im <- im(svm_mat)
plot(svm_mat_im, main = "SVM")

rf_mat <- matrix(rf_pred, nrow=172, ncol=114)
rf_mat_im <- im(rf_mat)
plot(rf_mat_im, main = "Random Forest")

boost_mat <- matrix(boost.pred.smote, nrow=172, ncol=114)
boost_mat_im <- im(boost_mat)
plot(boost_mat_im, main = "Boosting")

knn_mat <- matrix(knn.smote, nrow = 172, ncol = 114) 
...
model_performance <- data.frame(
  ...
  Balanced_Accuracy = c(84.04,83.94, 66.13, 57.69, 98.08),
  Specificity = c(91.15, 90.96, 91.87, 93.97, 100.00),
  Sensitivity = c(76.92, 76.92, 40.38, 57.69, 99.04)
)
